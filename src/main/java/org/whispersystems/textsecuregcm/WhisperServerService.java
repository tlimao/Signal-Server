...
public class WhisperServerService extends Application<WhisperServerConfiguration> {
      ...
      throws Exception
  {
    ...
    if (config.getApnConfiguration().isEnable()) {
      apnSender                  = new APNSender(accountsManager, config.getApnConfiguration());
    }
    ...
    if (config.getApnConfiguration().isEnable()) {
      apnFallbackManager  = new ApnFallbackManager(pushSchedulerClient, apnSender, accountsManager);
    }

    SmsSender                smsSender           = null;
    if (config.getTwilioConfiguration().isEnable()) {
    ...
    environment.jersey().register(new AccountController(pendingAccountsManager, accountsManager, rateLimiters, smsSender, emailSender, messagesManager, turnTokenGenerator, config.getTestDevices(), config.getTestAccounts()));
